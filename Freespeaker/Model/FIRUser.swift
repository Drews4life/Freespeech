...
class FIRUser {
    ...
    class func currentUser () -> FIRUser? {
        if let _ = Auth.auth().currentUser {
            if let dictionary = UserDefaults.standard.object(forKey: kCURRENTUSER) {
                ...
                return FIRUser.init(_dictionary: dictionary as! NSDictionary)
            }
        }
    ...
    class func registerUserWith(email: String, password: String, firstName: String, lastName: String, avatar: String = "", completion: @escaping (_ error: Error?) -> Void ) {
        ...
        Auth.auth().createUser(withEmail: email, password: password, completion: { (firuser, error) in
            ...
            let fUser = FIRUser(_objectId: firuser!.user.uid, _pushId: "", _createdAt: Date(), _updatedAt: Date(), _email: firuser!.user.email!, _firstname: firstName, _lastname: lastName, _avatar: avatar, _loginMethod: kEMAIL, _phoneNumber: "", _city: "", _country: "")
        ...
        Auth.auth().signInAndRetrieveData(with: credential) { (firuser, error) in
            ...
            fetchCurrentUserFromFirestore(userId: firuser!.user.uid, completion: { (user) in
                ...
                if user != nil && user!.firstname != "" {
                    ...
                    let fUser = FIRUser(_objectId: firuser!.user.uid, _pushId: "", _createdAt: Date(), _updatedAt: Date(), _email: "", _firstname: "", _lastname: "", _avatar: "", _loginMethod: kPHONE, _phoneNumber: firuser!.user.phoneNumber!, _city: "", _country: "")
...
func saveUserToFirestore(fUser: FIRUser) {
...
func saveUserLocally(fUser: FIRUser) {
...
func fetchCurrentUserFromFirestore(userId: String, completion: @escaping (_ user: FIRUser?)->Void) {
    ...
    reference(.User).document(userId).getDocument { (snapshot, error) in
        ...
        if snapshot.exists {
            ...
            let user = FIRUser(_dictionary: snapshot.data()! as NSDictionary)
            completion(user)
        } else {
            completion(nil)
        }
...
func userDictionaryFrom(user: FIRUser) -> NSDictionary {
...
func getUsersFromFirestore(withIds: [String], completion: @escaping (_ usersArray: [FIRUser]) -> Void) {
    
    var count = 0
    var usersArray: [FIRUser] = []
...
func updateCurrentUserInFirestore(withValues : [String : Any], completion: @escaping (_ error: Error?) -> Void) {
    ...
    if let dictionary = UserDefaults.standard.object(forKey: kCURRENTUSER) {
        ...
        let currentUserId = FIRUser.currentId()
